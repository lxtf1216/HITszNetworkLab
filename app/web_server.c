...
void http_respond(tcp_conn_t *tcp_conn, char *url_path, uint16_t port, uint8_t *dst_ip, uint16_t dst_port) {
    ...
    if (!file) {
        ...
        // TODO: 发送 HTTP 状态行
        snprintf(resp_buffer, sizeof(resp_buffer), "HTTP/1.1 404 Not Found\r\n");
        ...
        // TODO: 发送 HTTP 内容类型
        snprintf(resp_buffer, sizeof(resp_buffer), "Content-Type: text/html; charset=UTF-8\r\n");
        tcp_send(tcp_conn, (uint8_t *)resp_buffer, strlen(resp_buffer), port, dst_ip, dst_port);


        // TODO: 发送 HTTP 内容长度
        snprintf(resp_buffer, sizeof(resp_buffer), "Content-Length: %zu\r\n", strlen(not_found_body));
        tcp_send(tcp_conn, (uint8_t *)resp_buffer, strlen(resp_buffer), port, dst_ip, dst_port);
    ...
    /* Step2 ：发送 HTTP 请求头 */
    // TODO: 发送 HTTP 状态行
    snprintf(resp_buffer, sizeof(resp_buffer), "HTTP/1.1 200 OK\r\n");
    tcp_send(tcp_conn, (uint8_t *)resp_buffer, strlen(resp_buffer), port, dst_ip, dst_port);
    // 发送 HTTP 连接信息
    sprintf(resp_buffer, "Connection: Keep-Alive\r\n");
    tcp_send(tcp_conn, (uint8_t *)resp_buffer, strlen(resp_buffer), port, dst_ip, dst_port);
    ...
    if (content_type && strlen(content_type) > 0) {
        snprintf(resp_buffer, sizeof(resp_buffer), "Content-Type: %s\r\n", content_type);
    } else {
        /* 默认类型 */
        snprintf(resp_buffer, sizeof(resp_buffer), "Content-Type: application/octet-stream\r\n");
    }
    tcp_send(tcp_conn, (uint8_t *)resp_buffer, strlen(resp_buffer), port, dst_ip, dst_port);
